...
def read_activities():
    ...
    cursor.execute("SELECT a.id, `user_id`, `upload_id`, `external_id`, `start_date_local`, `name`, `distance`, "
                   ...
                   "GROUP_CONCAT(p.id), GROUP_CONCAT(p.url_small), GROUP_CONCAT(p.url_big) "
        ...
        if len(data) > 0:
            ...
            for d in data:
                ...
                cursor.execute("INSERT INTO activities (`id`, `user_id`, `upload_id`, `external_id`, "
                               ...
                               "`polyline`   = VALUES(`description`), "
                               "`summary_polyline`   = VALUES(`description`), "
                    ...
                    if r3.ok and r4.ok:
                        ...
                        if len(data3) == len(data4):
                            for i in range(len(data3)):
                                cursor.execute("INSERT INTO photos (`id`, `activity_id`, `url_small`, `url_big`) "
                                               "VALUES (%s, %s, %s, %s) "
                                               "ON DUPLICATE KEY "
                                               "UPDATE "
                                               "id  = VALUES(id), "
                                               ...
                                               (data3[i]["unique_id"], data3[i]["activity_id"],
                                                data3[i]["urls"]["200"], data4[i]["urls"]["30000"])
                                               )
